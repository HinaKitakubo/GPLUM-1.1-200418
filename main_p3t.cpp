...
#pragma omp critical
    ...
    while(1){
...
#endif
        Energy e_now_before = e_now;     //追加
        
        ///////////////
        /*   Merge   */
        ///////////////
       PS::S32 n_remove_glb;   //追加
        if ( n_col ) {
       ...
       if (n_loc > 0){     //粒子が存在する場合
         const PS::S32 N_loc = system_grav.getNumberOfParticleLocal();

         for ( PS::S32 i=0; i < n_loc; i++ ){  //（粒子が存在する場合）繰り返す

           if ( N_loc == 1 ){   //存在する粒子が1個の場合（⇒相互作用エネルギーが 0）
             //for ( PS::S32 i=0; i<n_loc; i++ ){     //粒子吸収前の粒子情報を出力する
               //PS::ParticleSystem<FPGrav> system_grav;
//std::cout << "HELLO 1" << std::endl;
               ...
               kin_e = 0.5*(system_grav[i].mass)*(vel*vel);               //運動エネルギーを求める
               pot_e = -(1.0 * pos.x * FPGrav::m_sun)/(sqrt(pos*pos));    //中心BHのポテンシャルエネルギーを求める
               std::cout << "●kinematic energy = " << kin_e << std::endl;   //運動エネルギーの出力
               std::cout << "●potental energy = " << pot_e << std::endl;    //中心BHのポテンシャルエネルギーの出力

               itr_e = 0;

               std::cout << "●interaction energy = " << itr_e << std::endl;   //相互作用エネルギーの出力
               std::cout << "●total energy = " << kin_e + pot_e + itr_e << std::endl << std::endl;  //粒子1つにおける全エネルギーの出力
             }
         //}

         else if ( N_loc == 2 ){   //粒子が2個の場合 n_loc=2 （⇒相互作用エネルギーが存在する） 
             //for ( PS::S32 i=0; i<n_loc; i++ ){     //粒子吸収前の粒子情報を出力する
               //PS::ParticleSystem<FPGrav> system_grav;
//std::cout << "HELLO 2" << std::endl;
               PS::F64vec pos = system_grav[i].pos;
               PS::F64vec vel = system_grav[i].vel;
               //PS::F64vec pos_aft = system_grav[i+1].pos;   //i=0の時、i=1との相互作用エネルギーを求めるため定義した
               //PS::F64vec pos_bef = system_grav[i-1].pos;   //i=1の時、i=0との相互作用エネルギーを求めるため定義した
               std::cout << "ID : " << i << std::endl;
               std::cout << "■x, y, z = " << pos.x << ", " << pos.y << ", " << pos.z << std::endl;
               std::cout << "■vx, vy, vz = " << vel.x << ", " << vel.y << ", " << vel.z << std::endl << std::endl;
...
//std::cout << "HELLO 3" << std::endl;
                   PS::F64vec pos_aft = system_grav[i+1].pos;   //i=0の時、i=1との相互作用エネルギーを求めるため定義した
                   itr_e_0 = -(0.5 * system_grav[i].mass * system_grav[i+1].mass * 1.0)/(sqrt( (pos.x - pos_aft.x)*(pos.x - pos_aft.x) + (pos.y - pos_aft.y)*(pos.y - pos_aft.y) + (pos.z - pos_aft.z)*(pos.z - pos_aft.z) ));
               }
               else {   //i=1の時
//std::cout << "HELLO 4" << std::endl;
                   PS::F64vec pos_bef = system_grav[i-1].pos;   //i=1の時、i=0との相互作用エネルギーを求めるため定義した
                   itr_e_1 = -(0.5 * system_grav[i-1].mass * system_grav[i].mass * 1.0)/(sqrt( (pos_bef.x - pos.x)*(pos_bef.x - pos.x) + (pos_bef.y - pos.y)*(pos_bef.y - pos.y) + (pos_bef.z - pos.z)*(pos_bef.z - pos.z) ));
               }

//std::cout << "HELLO 5" << std::endl;
               std::cout << "●interaction energy = " << itr_e << itr_e_0 + itr_e_1 << std::endl;   //相互作用エネルギーの出力
               std::cout << "●total energy = " << kin_e + pot_e + itr_e << std::endl << std::endl;  //粒子1つにおける全エネルギーの出力
            }

//std::cout << "HELLO 6" << std::endl;
              //std::cout << " time_sys = " << time_sys + FPGrav::dt_tree << std::endl;
...
//std::cout << "HELLO 7" << std::endl;
           ...
           /*for ( PS::S32 i=0; i<n_loc; i++ ){     //粒子吸収後の粒子情報を出力する
               PS::ParticleSystem<FPGrav> system_grav;
               ...
               std::cout << "▲after : x, y, z = " << pos.x << ", " << pos.y << ", " << pos.z << std::endl;
               std::cout << "▲after : vx, vy, vz = " << vel.x << ", " << vel.y << ", " << vel.z << std::endl;
               std::cout << "time_sys = " << time_sys << std::endl;
           }*/
        }
...
//std::cout << "HELLO 13" << std::endl;
            ...
            e_now.calcEnergy(system_grav);
            PS::F64 e_change = e_now.etot - e_now_before.etot;   //追加
            e_now.edisp += e_change;           //追加

            std::cout << "e_now_before = " << e_now_before.etot << std::endl;
            std::cout << "e_now = " << e_now.etot << std::endl;
            std::cout << "e_change = " << e_change << std::endl;
            std::cout << "enow.edisp = " << e_now.edisp << std::endl;
            }
